...
function Otp(Opts, attrs) {
  this['@'] = {
    ...
    ts: moment(attrs.ts, OTP_TS_ISO8601, true).isValid()
      ? attrs.ts
      : moment().format(OTP_TS_ISO8601),
...
function Opts(attrs) {
  if (Object.values(OPTS_CH).indexOf(attrs.ch) > -1) {
    return;
  }
  this['@'] = { ch: Object.values(OPTS_CH).indexOf(attrs.ch) > -1 ? attrs.ch : OPTS_DEFAULT_CH };
}

const buildXml = function (verConst) {
  init(verConst);

  const TEST_PERSON = constants.TEST_DATA[0];

  const opts = new Opts({ ch: OPTS_DEFAULT_CH });
  const otp = new Otp(opts, {
    uid: TEST_PERSON.uid,
    tid: OTP_DEFAULT_TID,
    ac: TEST_AUA_CODE,
    sa: TEST_SUBAUA_CODE,
    ver: OTP_DEFAULT_VER(),
    txn: 'testTxn',
    lk: OTP_AUA_LICENSEKEY,
    type: OTP_TYPE.AADHAAR,
  });

  const otpXml = js2xml.parse('Otp', otp /* , {declaration: {include: false}} */);

  signer.sign(otpXml).then(function (sign) {
